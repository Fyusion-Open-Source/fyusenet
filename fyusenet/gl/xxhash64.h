...
//
// ALTERATION BEGIN
// This is the license text from the link above (as of July 2022):
...
public:
  ...
  XXHash64(const std::string& data,uint64_t seed = 0) {
    ...
    totalLength = 0;
    if (data.size() > 0) {
      bool ok = add((const void *)data.c_str(),data.size());
      if (!ok) THROW_EXCEPTION_ARGS(GLException,"Cannot hash input data");
    }
  }
  ...
  static uint64_t hash(const std::string& str,uint64_t seed) 
  {
    XXHash64 hasher(str,seed);
    return hasher.hash();
