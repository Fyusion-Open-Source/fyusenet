 ...
 * @param type Pixel data type to use, see pixtype
 * @param channels Number of channels per pixel
 * @param pool BasicTexturePool instance to use, may be \c nullptr
 ...
 */
Texture2D::Texture2D(int width, int height, pixtype type, int channels, BasicTexturePool *pool, bool lock) :
    ...
    assert(channels > 0);
    if (pool) {
        handle_ = pool->obtainTexture(width, height, channels, type, lock);
        fromPool_ = pool;
        assert(*(handle_.get()) != 0);
        glBindTexture(GL_TEXTURE_2D, *(handle_.get()));
        updateParams();
...
void Texture2D::download(T *target) {
    ...
    tmp.addTexture(GL_COLOR_ATTACHMENT0, *(handle_.get()), GL_TEXTURE_2D);
    if (dataType_ == UINT8) {
...
Texture3D::Texture3D(int width, int height, int depth, pixtype type, int channels, bool clear) : Texture(GL_TEXTURE_2D, channels, type),
    ...
    createHandle();
    assert(*(handle_.get()) != 0);
    glBindTexture(GL_TEXTURE_3D, *(handle_.get()));
    updateParams();
    if (clear) this->clear();
}
...
#endif
    if (fromPool_) fromPool_->releaseTexture(*(handle_.get()));
}
