...
void BufferManager::cleanup() {
    if (texturePool_.size()>0) {
        GLuint textures[texturePool_.size()];
        int pi=0;
        for (auto ti = texturePool_.begin(); ti != texturePool_.end(); ++ti,pi++) {
            textures[pi]=(*ti).id_;
        }
        glDeleteTextures(texturePool_.size(),textures);
        texturePool_.clear();
    }
    bufferPool_.clear();
...
void BufferManager::createCPUOutput(LayerBase *outputLayer, bool lock) {
    // TOOD (mw) support multiple output ports
    cpu::CPULayerInterface * cpuout = dynamic_cast<cpu::CPULayerInterface *>(outputLayer);
    if (!cpuout) {
...
/**
 * @brief Createa  a (set of) GPU output texture(s) and assign it to the outputs of a layer
 *
 ...
 * in the network.
 */
void BufferManager::createGPUOutput(gpu::GPULayerBase *outputLayer, GLint internalFormat,
                                    GLint pixelFormat, GLenum dataType) {
    const std::vector<BufferSpec>& outputs = outputLayer->getRequiredOutputBuffers();
    for (auto texit = outputs.begin() ; texit != outputs.end(); ++texit) {
        Texture ot = createTexture((*texit).width_, (*texit).height_,
                                   internalFormat, pixelFormat, dataType, BufferSpec::LINEAR);
        gpu::GPULayerBase *gpu = dynamic_cast<gpu::GPULayerBase *>(outputLayer);
        if (!gpu) THROW_EXCEPTION_ARGS(FynException,"Cannot assign output texture to non-GPU layer");
        gpu->addOutputTexture(ot.id_, (*texit).channelIndex_);
 ...
 *
 * @param lock Flag that controls whether or not a connected input texture should be marked as
...
void BufferManager::connectLayers(LayerBase *outputLayer, LayerBase *inputLayer, int port, bool lock) {
    ...
    if (matches.at(0).first.device_ == BufferSpec::COMP_STOR_GPU) {
        gpu::GPULayerBase * outlayer = dynamic_cast<gpu::GPULayerBase *>(outputLayer);
        gpu::GPULayerBase * inlayer = dynamic_cast<gpu::GPULayerBase *>(inputLayer);
 ...
 *
 * @param lock Flag that controls whether or not a connected input buffer should be marked as
...
void BufferManager::connectCPULayers(LayerBase *outLayer, LayerBase *inLayer, std::vector<std::pair<BufferSpec,BufferSpec>> & matches, int port, bool lock) {
    cpu::CPULayerInterface * cpuout = dynamic_cast<cpu::CPULayerInterface *>(outLayer);
    cpu::CPULayerInterface * cpuin = dynamic_cast<cpu::CPULayerInterface *>(inLayer);
    ...
    for (auto it = matches.begin(); it != matches.end(); ++it) {
        ...
        //---------------------------------------------------------
        if (cpuout->hasOutputBuffer(it->first.port_)) {
            CPUBuffer * outbuf = cpuout->getOutputBuffer(it->first.port_);
            if (it->first.usage_ == BufferSpec::RESIDUAL_SOURCE) {
                cpuin->setResidualBuffer(outbuf);
            }
            else {
                cpuin->setInputBuffer(outbuf, port);
            }
            ...
            if (index >= 0) {
                ...
                if (it->first.usage_ == BufferSpec::RESIDUAL_SOURCE) {
                    cpuin->setResidualBuffer(buf);
                }
                else {
                    cpuin->setInputBuffer(buf, port);
                }
                inLayer->addInputConnection(port, outLayer, it->first.port_);
                cpuout->addOutputBuffer(buf);
                outLayer->addOutputConnection(it->second.port_, inLayer, port);
                ...
                // Create a new buffer...
                //-------------------------------------------------------
                Buffer nb = createBuffer(it->second.width_, it->second.height_, it->second.channels_, it->second.internalFormat_);
                nb.lastInputLayer_ = inLayer->getNumber();
                nb.locked_ = lock;
                bufferPool_.push_back(nb);
                cpuout->addOutputBuffer(nb.buf_);
                if (it->first.usage_ == BufferSpec::RESIDUAL_SOURCE) {
                    cpuin->setResidualBuffer(nb.buf_);
                }
                else {
                    cpuin->setInputBuffer(nb.buf_, port);
                }
                inLayer->addInputConnection(port,outLayer, it->first.port_);
                outLayer->addOutputConnection(it->second.port_, inLayer, port);
            }
...
void BufferManager::connectGPULayers(gpu::GPULayerBase * outLayer, gpu::GPULayerBase * inLayer, std::vector<std::pair<BufferSpec,BufferSpec>> & matches, int port, bool lock) {    
    AsyncLayer * asy = dynamic_cast<AsyncLayer *>(outLayer);
    //---------------------------------------------------------
    ...
    for (auto it = matches.begin(); it != matches.end(); ++it) {
        ...
        if (outLayer->hasOutputTexture(it->first.channelIndex_)) {
            ...
            if ((index >= 0) && (!lock) && (!it->second.lock_)) {
                ...
                inLayer->addInputConnection(port, outLayer, it->first.port_);
                outLayer->addOutputTexture(tid,it->second.channelIndex_);
                outLayer->addOutputConnection(it->second.port_, inLayer, port);
                updateLayerUse(index,inLayer->getNumber(),lock);
            } else {
                //-------------------------------------------------------
                ...
                outLayer->addOutputTexture(nt.id_,it->second.channelIndex_);
                inLayer->addInputConnection(port, outLayer, it->first.port_);
                outLayer->addOutputConnection(it->second.port_, inLayer, port);
                //-------------------------------------------------------
...
std::vector<std::pair<BufferSpec,BufferSpec>> BufferManager::checkIOMatch(LayerBase *inputLayer, const std::vector<BufferSpec>& inputs,const std::vector<BufferSpec>& outputs, int inputPort) {
    ...
    for (auto it=inputs.begin(); it != inputs.end(); ++it) {
        ...
        for (auto ot=outputs.begin(); ot != outputs.end(); ++ot) {
            ...
            bool intermatch = (outspec.interpolation_ == inspec.interpolation_)||(((outspec.interpolation_==BufferSpec::ANY)||(inspec.interpolation_==BufferSpec::ANY)));
            ...
            if (devmatch && idxmatch && (outspec.width_ == inspec.width_) && (outspec.height_ == inspec.height_) && intermatch) {
                if ((outspec.device_ == BufferSpec::COMP_STOR_CPU) && (outspec.channels_ != inspec.channels_)) continue;
                if (outspec.usage_ != BufferSpec::GPU_DEST) {
                    // the source layer is not an upload layer, use 4-chan format for
                    // textures by default, regardless of the channels required
                    // reason: GLES cannot render to RGB textures
                    inspec.internalFormat_ = gpu::GPULayerBase::TEXTURE_IFORMAT_4;
                    inspec.format_ = gpu::GPULayerBase::TEXTURE_FORMAT_4;
                }
                if ((outspec.usage_ == BufferSpec::OES_DEST) || (outspec.internalFormat_ == inspec.internalFormat_)) {
                    result.push_back(std::make_pair(inspec, outspec));
                }
            }
        }
    }
 ...
 * @param height Requrested texture height
...
int BufferManager::findTexture(int inputLayer, int outputLayer, int width, int height,
                               GLint internalFormat, BufferSpec::interp interpolation) const {
    assert(inputLayer > outputLayer);
    for (int i=0; i < (int)texturePool_.size(); i++) {
        const Texture & tx = texturePool_.at(i);
        if ((tx.width_ == width) && (tx.height_ == height) && (tx.internalFormat_ == internalFormat) && ((interpolation == BufferSpec::ANY)||(tx.interpolation_ == interpolation))) {
...
BufferManager::Texture BufferManager::createTexture(int width, int height,
                                                    GLint internalFormat, GLuint format, GLuint type,
    ...
    switch (interpolation) {
        case BufferSpec::LINEAR:
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            break;
        case BufferSpec::NEAREST:
            // intentional fallthrough
        default:
            // ANY interpolation defaults to nearest
            interpolation = BufferSpec::NEAREST;
    ...
    int err = glGetError();
    if (err != GL_NO_ERROR) {
        THROW_EXCEPTION_ARGS(GLException,"Cannot parameterize texture (err=0x%x)",err);
    }
#endif
    int elemsize = 1;
    ...
    return Texture(texture, width, height, internalFormat, interpolation);
}
