...
struct ConvLayerBuilderTempl : GPULayerBuilderTempl<D> {
     ...
     */
    ConvLayerBuilderTempl(short kernel, const std::string& name) : GPULayerBuilderTempl<D>(name),
        kernel_(kernel) {
    }
...
/**
 * @brief Builder class for convolution-type layers for GPU 2D convolution layers
 *
