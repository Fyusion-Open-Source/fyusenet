/* ----------------------------------------------------------------------------
 * Transpose Conv 3x3 (shallow, vanilla)   Copyright (c) 2016-2022 Fyusion Inc.
...
vec4 multiply(in vec4 inpix,in int offset) {
#ifdef POST_BATCHNORM
...
void procAndSet(in vec4 inpix,in int offset, const in int bnscale) {
...
void main(void) {  
  ...
  procAndAdd(texture(inputLayer,texCoord+vec2(texStep.x,0)),1);
#endif
#if STEP == 3
  procAndSet(texture(inputLayer,texCoord-vec2(0,texStep.y)),0);
  procAndAdd(texture(inputLayer,texCoord+vec2(0,texStep.y)),1);
#endif
#if STEP == 4
  procAndSet(texture(inputLayer,texCoord-texStep),0);
  procAndAdd(texture(inputLayer,texCoord+vec2(texStep.x,-texStep.y)),1);
  procAndAdd(texture(inputLayer,texCoord+vec2(-texStep.x,texStep.y)),2);
  procAndAdd(texture(inputLayer,texCoord+texStep),3);
#endif
}
