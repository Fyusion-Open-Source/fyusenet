/* ----------------------------------------------------------------------------
 * Avg-Pooling (shallow)                   Copyright (c) 2016-2022 Fyusion Inc.
...
vec4 pool(in sampler2D inLayer,in ivec2 tc) {
  ...
  result += activate(texelFetch(inLayer, tc+ivec2(1,0), 0));
  result += activate(texelFetch(inLayer, tc+ivec2(0,1), 0));
  result += activate(texelFetch(inLayer, tc+ivec2(1,1), 0));
  return result / float(POOL_SIZE*POOL_SIZE);
#else
