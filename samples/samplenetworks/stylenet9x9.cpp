...
fyusion::fyusenet::CompiledLayers StyleNet9x9::buildLayers() {
    ...
    auto * conv1 = new gpu::ConvLayerBuilder(9,"conv1");
    ...
    auto * res31 = new gpu::ConvLayerBuilder(3,"res3_1");
    res31->shape(40,height_/4, width_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).context(context()).number(layer_ids::RES3_1);
    res31->push(factory);

    auto * res32 = new gpu::ConvLayerBuilder(3,"res3_2");
    res32->shape(40,height_/4, width_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).residual().context(context()).number(layer_ids::RES3_2);
    res32->push(factory);

    auto * res41 = new gpu::ConvLayerBuilder(3,"res4_1");
    res41->shape(40,height_/4, width_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).context(context()).number(layer_ids::RES4_1);
    res41->push(factory);

    auto * res42 = new gpu::ConvLayerBuilder(3,"res4_2");
    res42->shape(40,height_/4, width_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).residual().context(context()).number(layer_ids::RES4_2);
    res42->push(factory);

    auto * res51 = new gpu::ConvLayerBuilder(3,"res5_1");
    res51->shape(40,height_/4, width_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).context(context()).number(layer_ids::RES5_1);
    res51->push(factory);

    auto * res52 = new gpu::ConvLayerBuilder(3,"res5_2");
    res52->shape(40,height_/4, width_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).residual().context(context()).number(layer_ids::RES5_2);
    res52->push(factory);
    ...
    auto * deconv3 = new gpu::ConvLayerBuilder(9,"deconv3");
...
void StyleNet9x9::connectLayers(fyusion::fyusenet::CompiledLayers& layers, fyusion::fyusenet::BufferManager * buffers) {
    ...
    buffers->connectLayers(layers[layer_ids::RES2_2],  layers[layer_ids::RES3_1],0);
    buffers->connectLayers(layers[layer_ids::RES2_2],  layers[layer_ids::RES3_2],1);
    buffers->connectLayers(layers[layer_ids::RES3_1],  layers[layer_ids::RES3_2],0);
    buffers->connectLayers(layers[layer_ids::RES3_2],  layers[layer_ids::RES4_1],0);
    buffers->connectLayers(layers[layer_ids::RES3_2],  layers[layer_ids::RES4_2],1);
    buffers->connectLayers(layers[layer_ids::RES4_1],  layers[layer_ids::RES4_2],0);
    buffers->connectLayers(layers[layer_ids::RES4_2],  layers[layer_ids::RES5_1],0);
    buffers->connectLayers(layers[layer_ids::RES4_2],  layers[layer_ids::RES5_2],1);
    buffers->connectLayers(layers[layer_ids::RES5_1],  layers[layer_ids::RES5_2],0);
    buffers->connectLayers(layers[layer_ids::RES5_2],  layers[layer_ids::DECONV1],0);
...
#ifdef FYUSENET_MULTITHREADING
        if (async_) {
            gpu::DownloadLayer * down = (gpu::DownloadLayer *)layers[layer_ids::DOWNLOAD];
            assert(down);
