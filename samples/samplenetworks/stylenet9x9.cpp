...
fyusion::fyusenet::CompiledLayers StyleNet9x9::buildLayers() {
    ...
    res31->shape(40,width_/4,height_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).context(context()).number(RES3_1);
    res31->push(factory);

    gpu::ConvLayerBuilder * res32 = new gpu::ConvLayerBuilder(3,"res3_2");
    res32->shape(40,width_/4,height_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).residual().context(context()).number(RES3_2);
    res32->push(factory);

    gpu::ConvLayerBuilder * res41 = new gpu::ConvLayerBuilder(3,"res4_1");
    res41->shape(40,width_/4,height_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).context(context()).number(RES4_1);
    res41->push(factory);

    gpu::ConvLayerBuilder * res42 = new gpu::ConvLayerBuilder(3,"res4_2");
    res42->shape(40,width_/4,height_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).residual().context(context()).number(RES4_2);
    res42->push(factory);

    gpu::ConvLayerBuilder * res51 = new gpu::ConvLayerBuilder(3,"res5_1");
    res51->shape(40,width_/4,height_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).context(context()).number(RES5_1);
    res51->push(factory);

    gpu::ConvLayerBuilder * res52 = new gpu::ConvLayerBuilder(3,"res5_2");
    res52->shape(40,width_/4,height_/4,40).type(LayerType::CONVOLUTION2D).prefixAct(ActType::RELU).residual().context(context()).number(RES5_2);
    res52->push(factory);
